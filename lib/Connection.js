...
class Connection {
  constructor (opts = { url: new url_1.URL('http://localhost:9200') }) {
    this.url = opts.url
  ...
  setRole (role, enabled) {
    assert_1.default(~validRoles.indexOf(role), `Unsupported role: '${role}'`)
    assert_1.default(typeof enabled === 'boolean', 'enabled should be a boolean')
