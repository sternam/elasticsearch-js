...
class Transport {
  ...
  request (params, callback) {
    callback = once_1.default(callback)
    ...
    if (connection === null) {
      return callback(new errors_1.NoLivingConnectionsError('There are not living connections'), result)
    }
    ...
    const request = connection.request(params, (err, response) => {
      if (err !== null) {
        ...
        const error = err instanceof errors_1.TimeoutError
          ? err
          : new errors_1.ConnectionError(err.message, params)
      ...
      response.on('error', (err) => callback(new errors_1.ConnectionError(err.message, params), result))
      response.on('end', () => {
        ...
        if (ignoreStatusCode === false && statusCode >= 400) {
          callback(new errors_1.ResponseError(result), result)
        } else {
          // cast to boolean if the request method was HEAD
