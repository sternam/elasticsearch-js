...
class ConnectionPool {
  constructor (opts = { Connection: Connection_1.default }) {
  ...
  markDead (connection) {
    ...
    connection.resurrectTimeout = Date.now() + Math.pow(this.resurrectTimeout, Math.min(connection.deadCount - 1, this.resurrectTimeoutCutoff))
  ...
  update (connections) {
    ...
    for (var i = 0; i < connections.length; i++) {
      ...
      if (this.connections.has(connection.id) === true) {
        ...
        if (oldConnection.status === Connection_1.default.statuses.DEAD) {
...
exports.internals = { defaultNodeFilter, roundRobinSelector, randomSelector }
