...
export default class ConnectionPool {
  ...
  constructor (opts: ConnectionPoolOptions = { Connection }) {
  ...
  markAlive (connection: Connection): void {
    ...
    const index: number = this.dead.indexOf(id)
  ...
  markDead (connection: Connection): void {
    ...
    this.dead.sort((a, b) => {
      const conn1: Connection = this.connections.get(a)
      const conn2: Connection = this.connections.get(b)
   ...
   */
  resurrect (now: number = Date.now(), callback: (isAlive: boolean | null, connection: Connection | null) => void = noop): void {
    ...
    const connection: Connection = this.connections.get(this.dead[0])
  ...
  addConnection (opts: any): Connection | void {
    ...
    const connection: Connection = new this.Connection(opts)
  ...
  removeConnection (connection: Connection): ConnectionPool {
    ...
    var index: number = this.dead.indexOf(id)
    if (index > -1) this.dead.splice(index, 1)
    return this
  }
  ...
  update (connections: Connection[]): ConnectionPool {
    ...
    for (var i = 0; i < connections.length; i++) {
      ...
      if (this.connections.has(connection.id) === true) {
        ...
        const oldConnection: Connection = this.connections.get(connection.id)
    ...
    const ids: string[] = connections.map(c => c.id)
  ...
  nodesToHost (nodes: any): any[] {
    ...
    for (var i = 0, len = ids.length; i < len; i++) {
      ...
      hosts.push({
        url: new URL(address),
        id: ids[i],
        roles: node.roles.reduce((acc: any, role: string) => {
          acc[role] = true
          return acc
        }, {})
      })
...
function randomSelector (connections: Connection[]): Connection {
  const index = Math.floor(Math.random() * connections.length)
  return connections[index]
}

export const internals = { defaultNodeFilter, roundRobinSelector, randomSelector }
